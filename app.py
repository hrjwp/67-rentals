...
def report():
    ...
    if request.method == "POST":
        ...
        for idx, file in enumerate(photos, start=1):
            ...
            print(
                f"SECURITY: Incident report file {idx} validated successfully. Filename: {file.filename}, Type: {detected_type}, Size: {original_size} bytes")
            ...
            try:
                ...
                print(
                    f"SECURITY: File {idx} processed - Encrypted: {file_metadata['encrypted']}, Watermarked: {file_metadata.get('watermarked', False)}")
            except Exception as e:
...
def test_encryption():
    ...
    try:
        ...
        return jsonify({
            ...
            "message": "Encryption is working!" if (
                        test_value == decrypted and encryption_key_set) else "Encryption may not be configured correctly"
        })
    except Exception as e:
        return jsonify({
            "status": "error",
    ...
    def backup_worker():
        ...
        while True:
            try:
                ...
                print(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ✅ Backup completed: {backup_info['backup_filename']} ({backup_info['backup_size_mb']} MB)")
    ...
    def retention_worker():
        ...
        while True:
            try:
                ...
                if result.get("skipped_reason"):
                    print(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ℹ️  Retention skipped: {result['skipped_reason']}")
                else:
                    print(
...
def start_schedulers():
    ...
    def retention_worker():
        while True:
            try:
                run_retention_purge(reason='auto')
            except Exception as e:
